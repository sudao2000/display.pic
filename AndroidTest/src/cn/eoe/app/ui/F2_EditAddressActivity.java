...
public class F2_EditAddressActivity extends BaseActivity implements BusinessResponse {
	...
	public static final int ADDRESS_SETDEFAULT = 2;
	public static final int ADDRESS_DELETE = 3;
	public static final int ADDRESS_UPDATE = 4;
	...
	public void OnMessageResponse(int resultCode) {
		if(resultCode == ADDRESS_INFO) {
			setAddressInfo();
		} else if(resultCode == ADDRESS_SETDEFAULT) {
            finish();
		} else if(resultCode == ADDRESS_DELETE) {
			delete();
			finish();
		} else if(resultCode == ADDRESS_UPDATE) {
