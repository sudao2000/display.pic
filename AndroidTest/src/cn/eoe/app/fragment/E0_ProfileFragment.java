...
public class E0_ProfileFragment extends Fragment implements IXListViewListener,
		OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		image.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				 ...
				 * G0_SettingActivity.class); startActivity(intent);
		...
		memberLevelIcon = (ImageView) headView
				.findViewById(R.id.member_level_icon);

		setting = (ImageView) headView.findViewById(R.id.profile_head_setting);
		// photo = (WebImageView)
		// headView.findViewById(R.id.profile_head_photo);
		...
		history = (FrameLayout) headView
				.findViewById(R.id.profile_head_history);
		history_num = (TextView) headView
				.findViewById(R.id.profile_head_history_num);

		collect = (LinearLayout) headView
				.findViewById(R.id.profile_head_collect);
		...
		logout = (LinearLayout) headView.findViewById(R.id.profile_logout);
		...
		/*
		 * if (null == userInfoModel) { userInfoModel = new
		 * UserInfoModel(getActivity()); }
		 * userInfoModel.addResponseListener(this); if (uid.equals("")) {
		 * 
		 * Resources resource = mContext.getResources();
		 * 
		 * String click=resource.getString(R.string.click_to_login);
		 * name.setText(click); camera.setVisibility(View.GONE);
		 * memberLevelLayout.setVisibility(View.GONE);
		 * 
		 * } else { userInfoModel.getUserInfo();
	...
	public void setUserInfo() {
		/*
		 * name.setText(user.name); File files = new
		 * File(getActivity().getCacheDir
		 * ()+"/ECMobile/cache"+"/"+uid+"-temp.jpg");
		 * if(files.exists()&&!uid.equals("")) {
		 * photo.setImageBitmap(BitmapFactory
		 * .decodeFile(getActivity().getCacheDir
		 * ()+"/ECMobile/cache"+"/"+uid+"-temp.jpg")); } else {
		 * photo.setImageResource(R.drawable.profile_no_avarta_icon); }
		 * memberLevelName.setText(user.rank_name);
		 * memberLevelLayout.setVisibility(View.VISIBLE); Resources resource =
		 * mContext.getResources(); if (user.rank_level !=
		 * UserInfoModel.RANK_LEVEL_NORMAL) {
		 * memberLevelIcon.setVisibility(View.VISIBLE); } else {
		 * memberLevelIcon.setVisibility(View.GONE); }
		 * 
		 * if(!user.order_num.await_pay.equals("0")) {
		 * payment_num.setVisibility(View.VISIBLE);
		 * payment_num.setText(user.order_num.await_pay); } else {
		 * payment_num.setVisibility(View.GONE); }
		 * 
		 * if(!user.order_num.await_ship.equals("0")) {
		 * ship_num.setVisibility(View.VISIBLE);
		 * ship_num.setText(user.order_num.await_ship); } else {
		 * ship_num.setVisibility(View.GONE); }
		 * 
		 * if(!user.order_num.shipped.equals("0")) {
		 * receipt_num.setVisibility(View.VISIBLE);
		 * receipt_num.setText(user.order_num.shipped); } else {
		 * receipt_num.setVisibility(View.GONE); }
		 * 
		 * if(!user.order_num.finished.equals("0")) {
		 * history_num.setVisibility(View.VISIBLE);
		 * history_num.setText(user.order_num.finished); } else {
		 * history_num.setVisibility(View.GONE); }
		 * if(user.collection_num.equals("0")){
		 * collect_num.setText(resource.getString(R.string.no_product)); }else{
		 * collect_num
		 * .setText(user.collection_num+resource.getString(R.string.no_of_items
		 * )); }
		 */
	}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		 ...
		 * 
		 * if(requestCode == 1) { String sdStatus =
		 * Environment.getExternalStorageState(); Bundle bundle =
		 * data.getExtras(); Bitmap bitmap = (Bitmap) bundle.get("data");//
		 * 获取相机返回的数据，并转换为Bitmap图片格式
		 * 
		 * if (file == null) { file = new File(ProtocolConst.FILEPATH); if
		 * (!file.exists()) { file.mkdirs(); } } FileOutputStream b = null;
		 * String fileName =
		 * getActivity().getCacheDir()+"/ECMobile/cache"+"/"+uid+"-temp.jpg";
		 ...
		 * e.printStackTrace(); } } ((ImageView)
		 * view.findViewById(R.id.profile_head_photo)).setImageBitmap(bitmap);//
		 * 将图片显示在ImageView里 }
		 * 
		 * }
		 * 
		 * if(requestCode == 2) { userInfoModel.getUserInfo(); }
		 */
	}
	/*
	 * public void OnMessageResponse(String url, JSONObject jo, AjaxStatus
	 * status) { if (url.endsWith(ApiInterface.USER_INFO)) {
	 * xlistView.stopRefresh(); xlistView.setRefreshTime(); user =
	 * userInfoModel.user; // 从网络获取 setUserInfo(); } }
	 * 
	 * 
	 * public static USER userInfo(String uid) { return new
	 * Select().from(USER.class).where("USER_id = ?", uid).executeSingle(); }
	 * 
	 * @Override public void onDestroy() {
	 * userInfoModel.removeResponseListener(this); super.onDestroy(); }
	 * 
	 * @Override public void onPause() {
	 * 
	 * super.onPause(); MobclickAgent.onPageEnd("Profile"); }
	 */
}
